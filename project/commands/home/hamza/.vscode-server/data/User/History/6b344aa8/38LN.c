...
int main()
{
    ...
    else
    {
        ...
        // array for storing the line
        char str[11];
        // counter to get the number of bytes red from the file - may not be 10 every time as the total characters in file may not be a multiple of 10
        int bytes;
        // counter for size of file
        int file_size = 0;

        // print the contents of the file 10 bytes at a time (line by line)
        while ((bytes = read(file, str, 10)) > 0)
        {
            // replace last element with null if last element is less then 10 so we have the exactly the contents of file and no garbage
            str[bytes] = '\0';
            printf("%s\n", str);
